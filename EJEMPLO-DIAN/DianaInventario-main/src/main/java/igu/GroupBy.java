...
public class GroupBy {
    ...
    private boolean cargandoComboCategorias;
    ...
    public GroupBy(JFrame parent, SessionContext session) {
        ...
        comboBoxCategoria.addActionListener(e -> {
            if (!enVerificacion) {
                if (cargandoComboCategorias) {
                    return;
                }
                String seleccion = (String) comboBoxCategoria.getSelectedItem();
                if (seleccion == null || seleccion.isBlank() || "Seleccione categoria...".equalsIgnoreCase(seleccion)) {
                    return;
                }
                cargarJoyasPorCategoria(seleccion);

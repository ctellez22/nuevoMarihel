...
public class VerDatos {
    ...
    private void inicializarOpcionesDeFiltro() {
        ...
        cmbEstado.setSelectedItem("Estado: Disponible");
    }
    ...
    private void cargarFiltrosDesdeBD() {
        ...
        for (Socio socio : controladora.obtenerSocios(session)) {
    ...
    private void ejecutarFiltro(boolean mostrarAviso) {
        ...
        filtroWorker = new SwingWorker<>() {
            ...
            protected List<Joya> doInBackground() {
                ...
                boolean incluirPendientesInsert = session != null && session.isAdmin();
    ...
    private Joya mapearPendienteInsertAJoya(CambioPendiente pendiente) {
        ...
        Joya joya = new Joya();
        Long entidadId = pendiente.getEntidadId();
        joya.setId(entidadId == null ? -Math.abs(pendiente.getId()) : -Math.abs(entidadId));
        String displayId = extraerTexto(json, "displayId");
        joya.setDisplayId(displayId.isBlank() ? "" : displayId);
        ...
        joya.setPuntoFisico(extraerTexto(json, "puntoFisico"));
        ...
        joya.setPrecioVentaReal(extraerTexto(json, "precioVentaReal"));
    ...
    private boolean cumpleFiltrosPendiente(Joya joya,
        ...
        if (session != null && !session.isAdmin()) {
            String puntoSesion = (session.puntoFisico() == null) ? "" : session.puntoFisico().trim();
            String puntoJoya = (joya.getPuntoFisico() == null) ? "" : joya.getPuntoFisico().trim();
            if (puntoSesion.isBlank() || !puntoSesion.equalsIgnoreCase(puntoJoya)) {
                return false;
            }
        }
